    ...
    <div className="space-y-4">
      ...
      <div className="card-glass p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <h2 className="text-xl font-bold text-foreground">Historial de Custos</h2>
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1">
              <Filter className="w-3.5 h-3.5 text-muted-foreground" />
              <select value={filterType} onChange={e => setFilterType(e.target.value as any)} className="input-field !py-1.5 !text-xs w-[100px]">
            ...
            <div className="flex items-center gap-2">
              ...
              <a href={`/api/export/csv?type=costs&from=${dateFrom}&to=${dateTo}`} className="p-1.5 rounded-lg hover:bg-white/5 text-muted-foreground hover:text-foreground transition-colors" title="Exportar CSV">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5">
                  ...
                  <th className="text-left py-3 text-xs font-medium text-muted-foreground">Tipo</th>
                  <th className="text-left py-3 text-xs font-medium text-muted-foreground hidden sm:table-cell">Categoria</th>
                  <th className="text-left py-3 text-xs font-medium text-muted-foreground hidden md:table-cell">Descrição</th>
                  <th className="text-right py-3 text-xs font-medium text-muted-foreground">Valor</th>
                    ...
                    {editingId === entry.id ? (
                      <>
                        ...
                        <td className="py-3 hidden md:table-cell">
                          <input type="text" value={editForm.description} onChange={e => setEditForm({ ...editForm, description: e.target.value })} className="input-field !py-1 !text-xs w-full" />
                        </td>
                        <td className="py-3 text-right">
                          <input type="number" step="0.01" value={editForm.amount} onChange={e => setEditForm({ ...editForm, amount: parseFloat(e.target.value) || 0 })} className="input-field !py-1 !text-xs !text-right w-24 ml-auto" />
                        </td>
                        <td className="py-3 text-right">
              ...
              <tfoot>
                <tr className="border-t border-white/10">
                  <td colSpan={4} className="py-3 text-xs font-bold text-foreground">Total ({entries.length} entradas)</td>
