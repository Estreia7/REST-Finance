...
export default function RestaurantDetailPanel({ restaurantId, onBack }: RestaurantDetailPanelProps) {
  ...
  const handleSaveDetail = async () => {
    ...
    if (result.success) { toast.success('Restaurante atualizado!'); await loadDetail(); }
  ...
  const handleEditEntry = async (type: 'revenue' | 'cost', id: string) => {
    ...
    if (result.success) { toast.success('Atualizado!'); setEditingId(null); await loadEntries(); }
    else toast.error(result.error || 'Erro');
    setSaving(false);
  };
  ...
  const handleDeleteEntry = async (type: 'revenue' | 'cost', id: string) => {
    ...
    if (result.success) { toast.success('Eliminado!'); await loadEntries(); }
    else toast.error(result.error || 'Erro');
  };

  const fmt = (n: number) => `€${n.toLocaleString('pt-PT', { minimumFractionDigits: 2 })}`;
  const fmtDate = (d: Date) => new Date(d).toLocaleDateString('pt-PT');

  if (loading) {
    return <div className="flex items-center justify-center py-16"><Loader2 className="w-6 h-6 animate-spin text-muted-foreground" /></div>;
  }

  if (!detail) return <div className="text-center py-16 text-muted-foreground">Restaurante não encontrado.</div>;
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="card-glass p-4 text-center">
              ...
              <div className="text-[10px] text-muted-foreground">Receita este mês</div>
            </div>
            <div className="card-glass p-4 text-center">
              ...
              <div className="text-[10px] text-muted-foreground">Custos este mês</div>
            </div>
            <div className="card-glass p-4 text-center">
          ...
          <div className="card-glass p-6">
            <h3 className="text-sm font-bold text-foreground mb-4">Editar Restaurante</h3>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={editPlan} onChange={e => setEditPlan(e.target.value)} className="input-field">
                ...
                <div>
                  <label className="text-xs text-muted-foreground block mb-1">Trial até</label>
                  <input type="date" value={editTrial} onChange={e => setEditTrial(e.target.value)} className="input-field" />
                </div>
              </div>
              <button onClick={handleSaveDetail} disabled={savingDetail} className="cta-button py-2 px-4 text-sm">
                {savingDetail ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Guardar'}
              </button>
            </div>
          </div>
      ...
      {(tab === 'revenue' || tab === 'costs') && (
        <div className="space-y-4">
          ...
          {entriesLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground text-sm">Sem entradas.</div>
          ) : (
                    ...
                    <tr key={e.id} className="border-b border-border-subtle hover:bg-muted">
                      ...
                      <td className="py-2 px-4 text-right">
                        <div className="flex gap-1 justify-end">
                          <button onClick={() => handleDeleteEntry(tab === 'revenue' ? 'revenue' : 'cost', e.id)} className="p-1 rounded hover:bg-danger/10 text-muted-foreground hover:text-red-400">
